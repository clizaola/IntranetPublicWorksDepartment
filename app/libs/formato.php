<?php
	class Formato{
		...
		public static function tiempo($fecha=""){
	        ...
	        if(strpos($fecha,"/")>0){
	        	if(strlen($fecha)>10){
	        		return mktime(substr($fecha,11,2),substr($fecha,14,2),substr($fecha,17,2),substr($fecha,3,2),substr($fecha,0,2),substr($fecha,6,4));
	        	}
				else{
			...
			if(strpos($fecha,"-")>0){
				if(strlen($fecha)>10){
	        		return mktime(substr($fecha,11,2),substr($fecha,14,2),substr($fecha,17,2),substr($fecha,5,2),substr($fecha,8,2),substr($fecha,0,4));
	        	}
				else{
		...
		public static function tiempoFecha($fecha){
			$tiempo = Formato::tiempo($fecha);
		}
