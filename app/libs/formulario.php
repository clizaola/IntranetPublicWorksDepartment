<?php
	...
	class Formulario{
        ...
        public static function checkbox($nombre, $activo = 0, $disabled = false){
        	return '<input type="checkbox" name="'.$nombre.'" '.($disabled ? 'disabled="disabled"' : "").' '.($activo ? "CHECKED" : "").'>';
        }
        ...
        public static function selectModelo($nombre, $registros, $opcion="id", $valor="id", $seleccion=""){
            ...
            if(!isset($params["include_blank"]) && $seleccion!="***"){
            	$tmp[""] = "...";
			}
        ...
        public static function autocompleteModelo($nombre, $opciones, $opcion="id", $valor=""){
			...
			if(!isset($params['value']))  $params["value"] = utf8_encode($valor);
        	...
        	$code = '<script>
					...
					</script><div class="ui-widget">'.($params["case"]=="mayusculas" ? Formulario::mayusculas($params) : Formulario::texto($params)).'</div>';
        ...
        public static function textarea($nombre,$valor=""){
			...
			return  @str_replace("###", ":", textarea_tag($params));
        }
